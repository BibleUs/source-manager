        ...
        for(; i < j; i++) {
            ...
            while(true) {
                ...
                if(val === undefined) {
                    ...
                    let exists = typeof stack[stack.length-1][key];

                    if ( exists != 'undefined' ) {
                        if ( exists == 'string' ) {
                            let oldvalue = stack[stack.length-1][key];
                            stack[stack.length-1][key] = [];
                            stack[stack.length-1][key].push(oldvalue);
                        }
            ...
            if( typeof obj[key] == "object" && Object.prototype.toString.call(obj[key]) == '[object Object]' ) {
                ...
                if ( Object.prototype.toString.call(obj[key]) == '[object Array]' ) {
                    for( var i in obj[key] ) {
                        if ( String(obj[key][i]).indexOf('function') == -1 )
                            buf += [line_indent, '"', key, '" "', String(obj[key][i]), '"\n'].join('');
                    }
                }
                else {
